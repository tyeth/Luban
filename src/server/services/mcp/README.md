...
| `mcpHttpsCert`, `mcpHttpsKey` | Absolute paths to a PEM certificate and its private key, e.g. generated by mkcert. Setting both adds HTTPS on `mcpPort + 1` (default 40890); HTTP stays on its original port. Settings → MCP Server edits these paths; env `LUBAN_MCP_HTTPS_CERT` / `LUBAN_MCP_HTTPS_KEY` override them independently (an explicitly empty env value clears that path). Applies at the next start. |
...
HTTP port and the next port). Both listeners use the same routes, registry, jobs, approval
state, camera and loopback/LAN/Origin checks. HTTPS does not enable LAN access or add
login authentication. No redirect replaces the existing HTTP endpoint, so existing agents
continue to work. Generated browser/confirmation links prefer HTTPS once it is listening;
camera-page assets stay on the origin the browser opened. OAuth metadata advertises that
listener's scheme and port, including all token/authorization endpoints.

One-time setup, on the computer that will run Luban (schedule its restart when no job is running):
...
2. Generate the server certificate for localhost **and the actual LAN IPv4 address** you will
   ...
   ```sh
   mkdir -p "$HOME/.local/share/luban/mcp-tls"
   chmod 700 "$HOME/.local/share/luban/mcp-tls"
   mkcert -cert-file "$HOME/.local/share/luban/mcp-tls/mcp-cert.pem" \
     -key-file "$HOME/.local/share/luban/mcp-tls/mcp-key.pem" \
     localhost 127.0.0.1 ::1 192.168.1.153
   chmod 600 "$HOME/.local/share/luban/mcp-tls/mcp-key.pem"
   ```

   On Windows, create a private directory and use the same mkcert flags with Windows paths.
   Keep the certificate/key outside the repository. Use a DHCP reservation, or regenerate
   the certificate when the LAN address changes. Use numeric LAN URLs: the existing browser
   Origin gate allows local subnet IPs and localhost, not arbitrary DNS/mDNS names.
3. In **Settings → MCP Server → HTTPS and browser notifications**, enter the absolute paths
   to those two files, save, and restart Luban when safe. Alternatively set the environment
   before launching the server (desktop launchers may not inherit shell variables):

   ```sh
   export LUBAN_MCP_HTTPS_CERT="$HOME/.local/share/luban/mcp-tls/mcp-cert.pem"
   export LUBAN_MCP_HTTPS_KEY="$HOME/.local/share/luban/mcp-tls/mcp-key.pem"
   ```

   Enable the existing LAN setting for phone access. The Settings pane shows the live HTTPS
   status, errors and URLs, and links to the dashboard. `GET /api/mcp` also reports `https`
   (`running`, actual `port`, configured paths/port, `error`, `mcpUrl`, `jobsUrl`, `lanUrls`).
4. **Trust this CA on each phone/PC.** `mkcert -CAROOT` prints the directory containing
   `rootCA.pem`. Transfer **only that public CA certificate** to your own devices; never
   transfer `rootCA-key.pem` or the server private key. Follow mkcert's
   [mobile-device instructions](https://github.com/FiloSottile/mkcert#mobile-devices):
   on iOS, install the downloaded profile and enable full trust under Certificate Trust
   Settings; on Android, install it as a CA certificate in the device's certificate settings
   (browser trust support varies). Other PCs can import the same public root into their
   browser/system trust store. mkcert's leaf certificate is signed by your local CA; trusting
   the CA is what makes it usable without certificate errors.
5. Open `https://192.168.1.153:40890/jobs` on the phone and verify it loads without certificate
   errors. On iPhone/iPad, add the supported web app to the Home Screen, open it there, then
   enable system notifications for that session. No notification preference is enabled by
   the HTTPS setup. HTTP and HTTPS are separate browser origins, so permission/session
   preferences are separate too. HTTPS enables the notification API; it does not add delivery
   while the browser is asleep or closed.

Node-based MCP clients using HTTPS may also need
`NODE_EXTRA_CA_CERTS` pointing to that device's copy of `rootCA.pem`, set before the client
starts. Do not disable certificate verification. HTTP clients need no change.

No certificate paths means HTTP-only. A partial pair, unreadable/invalid/mismatched PEM files,
port 65535 (no room for +1), or an occupied HTTPS port leaves HTTP working and reports an HTTPS
error. Certificates load at startup: restart after replacement or renewal. Clearing both stored
paths and removing any env overrides disables HTTPS at the next restart. Both listeners and
their persistent sockets close on service shutdown. No certificates or CA keys are bundled.
...
Use the HTTPS listener described above for system notifications on LAN clients. The existing
numeric LAN/loopback Origin checks still apply; this is not an internet-facing service. Reliable
locked-phone notifications would be a separate Web Push feature with explicit subscription/session
expiry semantics.
...
Own `http.Server` plus an optional `https.Server` on port + 1, bound to loopback by default (`mcpAllowLan` widens it to this machine's own IPv4
