...
export const tests: Array<[string, () => void]> = [
    ...
    ['a wedged bridge is named as one, and says to replug it', () => {
        ...
        assert.ok(/enumerated on USB but not servicing requests/.test(text));
        assert.ok(/UNPLUG AND REPLUG/.test(text));
        assert.ok(/nothing in software can reset it/.test(text), 'stops anyone hunting for a software fix');
    ...
    ['nothing at all arrived: that is the interpreter or the install, not the board', () => {
        ...
        assert.ok(!/REPLUG/.test(text), 'replugging fixes nothing when Blinka never loaded');
    }],
